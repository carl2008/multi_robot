...
def chooseFrontier():
    ...
    if (map):
        ...
        for index, point in enumerate(data):
            ...
            if (point == -1): # and isFrontierReachable([robotX, robotY], [frontierX, frontierY])):
                frontier = Pose()
                ...
                print("Found frontier " + str(frontier.position))
                return frontier
    return None
...
def moveToMapPose(pose: Pose, wait: Bool = False):
    ...
    if wait:
        wait_for_result = moveBaseClient.wait_for_result(timeout = rospy.Duration(10))
